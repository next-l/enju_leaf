class PatronMergesController < ApplicationController
  ...
  def index
    if @patron
      @patron_merges = @patron.patron_merges.page(params[:page]).order('patron_merges.id')
    elsif @patron_merge_list
      @patron_merges = @patron_merge_list.patron_merges.page(params[:page]).order('patron_merges.id')
    else
