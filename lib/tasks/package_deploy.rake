...
namespace :enju_trunk do
  namespace :pack do
    ...
    task :staging => :environment do
      sh "cd #{::Rails.root}; git log -1 > GitLastLog"
      archives = "Gemfile Gemfile.lock GitLastLog Rakefile app/ config/environments/ db/fixtures/ config/locales/ config/routes.rb db/ lib/ public/ script/ vendor/fonts vendor/cache/"
      excludes = "*.sample"
