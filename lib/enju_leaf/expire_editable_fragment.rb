module ExpireEditableFragment
  ...
  def expire_manifestation_cache(manifestation, fragments = [], formats = [])
    fragments = %w[detail pickup book_jacket title picture_file show_list edit_list reserve_list bookmark_list] if fragments.size == 1 and fragments.first == 'detail'
