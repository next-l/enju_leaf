...
describe Basket do
  ...
  it "should not create basket when user is not active" do
    expect(Basket.create(user: users(:user4)).id).to be_nil
  end

  it "should save shelf and library" do
    expect(items(:item_00011).checkouts.count).to eq 0
    basket_1 = Basket.new
    ...
    expect(items(:item_00011).checkouts.order('id DESC').first.shelf.name).to eq items(:item_00001).shelf.name
    expect(items(:item_00011).checkouts.order('id DESC').first.library.name).to eq users(:librarian1).profile.library.name
  end
  ...
  it "should not check out items that are already checked out" do
    ...
    checked_item_2.save
    basket_1.basket_checkout(users(:librarian1))
    expect { basket_2.basket_checkout(users(:librarian1)) }.to raise_exception ActiveRecord::RecordInvalid
    expect(items(:item_00011).checkouts.order('id DESC').first.user).to eq users(:admin)
  end
  ...
  it "should change reservation status" do
    ...
    checked_item.save
    expect(checked_item.item.circulation_status.name).to eq 'Available On Shelf'
    basket.basket_checkout(users(:librarian1))
    expect(checked_item.item.circulation_status.name).to eq 'On Loan'
  end
