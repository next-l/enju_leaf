...
RUN apt-get update -qq && apt-get install --no-install-recommends -y curl gnupg && \
  ...
  echo "deb [signed-by=/etc/apt/keyrings/yarnkey.gpg] https://dl.yarnpkg.com/debian stable main" | tee /etc/apt/sources.list.d/yarn.list && \
    apt-get update -qq && \
    apt-get install --no-install-recommends -y build-essential git libpq-dev pkg-config nodejs yarn shared-mime-info
...
RUN apt-get update -qq && apt-get install --no-install-recommends -y curl gnupg && \
  ...
  echo "deb [signed-by=/etc/apt/keyrings/yarnkey.gpg] https://dl.yarnpkg.com/debian stable main" | tee /etc/apt/sources.list.d/yarn.list && \
    apt-get update -qq && \
    apt-get install --no-install-recommends -y libvips postgresql-client nodejs yarn file imagemagick poppler-utils ffmpeg && \
