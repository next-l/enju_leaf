...
class ResourceTest < ActiveSupport::TestCase
  ...
  def test_reserved
    assert resources(:manifestation_00007).is_reserved_by(users(:admin))
  end

  def test_not_reserved
    assert_nil resources(:manifestation_00007).is_reserved_by(users(:user1))
  end

  def test_manifestation_should_show_languages
    assert resources(:manifestation_00001).language
  end
  ...
  def test_manifestation_should_get_number_of_pages
    assert_equal 100, resources(:manifestation_00001).number_of_pages
  end

  def test_manifestation_should_import_isbn
    assert Resource.import_isbn('4797327030')
  end

  def test_youtube_id
    assert_equal resources(:manifestation_00022).youtube_id, 'BSHBzd9ftDE'
  end

  def test_nicovideo_id
    assert_equal resources(:manifestation_00023).nicovideo_id, 'sm3015373'
  end
  ...
  def test_manifestation_should_respond_to_title
    assert resources(:manifestation_00001).title
  end

  def test_manifestation_should_have_screen_shot
    assert resources(:manifestation_00003).screen_shot
  end

  def test_manifestation_should_not_have_parent_of_series
    assert resources(:manifestation_00001).parent_of_series
  end

  def test_manifestation_should_response_to_extract_text
    assert_nil resources(:manifestation_00001).extract_text
  end
